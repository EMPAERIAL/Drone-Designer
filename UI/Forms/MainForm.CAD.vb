...
'   • _lastSelectionResult — Field holding the last SelectionResult (added below
...
Namespace Drone_Designer.UI.Forms
    ...
    Partial Public Class MainForm
        ...
        ' NOTE: _lastSelectionResult is declared in MainForm.Logic.vb (Task 11).
        ' If for any reason it wasn't added there, uncomment this line:
        ' Private _lastSelectionResult As SelectionResult = Nothing
        ...
        Private Sub WireCADControls()
            ...
            If swStatusItem IsNot Nothing Then
                AddHandler swStatusItem.Click, AddressOf CheckSolidWorksStatus_Click
            End If
        ...
        Private Async Sub btnGenerateCAD_Click(sender As Object, e As EventArgs)
            ...
            If _lastSelectionResult Is Nothing OrElse
               _lastSelectionResult.Motors Is Nothing OrElse
               _lastSelectionResult.Motors.Count = 0 Then
            ...
            Try
                ...
                If result.Success Then
                    UpdateStatus($"✓  {result.SuccessfulPartCount} CAD part(s) generated — " &
                                 $"{outputDir}")
        ...
        Private Function EnsureOrchestratorInitialised() As Boolean
            Try
                ...
                If _orchestrator Is Nothing Then
                    Dim config As New ConfigManager()
                    Dim macroRunner As New MacroRunner(_swAutomation.Application)

                    ' The selection engine is already instantiated in MainForm.Logic.vb
                    ' as _selectionEngine.  If the naming differs in your build, adjust here.
        ...
        Private Sub UpdateCADButtonState()
            Dim hasSelection As Boolean = _lastSelectionResult IsNot Nothing AndAlso
                                          _lastSelectionResult.Motors IsNot Nothing AndAlso
                                          _lastSelectionResult.Motors.Count > 0
